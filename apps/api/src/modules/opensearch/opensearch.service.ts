...
export class OpenSearchService implements OnModuleInit {
  ...
  constructor() {
    ...
    // Configuración vía variables de entorno (antes estaba hardcodeada a
    // localhost y una contraseña fija, lo que rompía en Docker/producción).
    this.client = new Client({
      node: process.env.OPENSEARCH_URL || 'https://localhost:9200',
      auth: {
        username: process.env.OPENSEARCH_USERNAME || 'admin',
        password: process.env.OPENSEARCH_PASSWORD || 'Ppp_os_password123!'
      },
      ssl: {
