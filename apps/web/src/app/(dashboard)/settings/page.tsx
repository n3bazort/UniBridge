                ...
                {periods.map(period => (
                  <div key={period.id} className={`p-6 rounded-[16px] border transition-all ${period.isActive ? 'border-indigo-200 bg-indigo-50/30 shadow-sm' : 'border-[#eef2f7] bg-white'}`}>
                    ...
                    {expandedPeriods[period.id] && (
                      ...
                      <div className="bg-white border border-[#eef2f7] rounded-[12px] p-4 flex flex-col gap-2">
                        <div className="flex items-center justify-between">
                          <label className="text-[11px] font-bold text-[#64748b] uppercase tracking-wider flex items-center gap-1.5">
                            <Users className="w-3.5 h-3.5" /> Decano(a) de la Facultad
                            {!period.deanName && <span className="text-red-500 normal-case font-medium">· requerido</span>}
                          </label>
                        </div>
                        {activeDeans.length > 0 && isAdmin && (
                          <div className="flex flex-col gap-1">
                            <select
                              ...
                              value={activeDeans.find((d: any) => d.fullName === period.deanName)?.fullName || ''}
                              className="w-full bg-indigo-50/60 border border-indigo-100 rounded-[8px] px-2.5 py-1 text-xs font-semibold text-indigo-900 cursor-pointer"
                            >
                              <option value="">-- Seleccionar Decano Oficial Activo --</option>
                              {activeDeans.map((d: any) => (
                                <option key={d.id} value={d.fullName}>
                                  {d.fullName} {d.facultyName ? `(${d.facultyName})` : ''}
                                </option>
                              ))}
                            </select>
                          </div>
                      ...
                      <div className="bg-white border border-[#eef2f7] rounded-[12px] p-4 flex flex-col gap-2">
                        <label className="text-[11px] font-bold text-[#64748b] uppercase tracking-wider flex items-center gap-1.5">
                          <Users className="w-3.5 h-3.5" /> Responsable de Prácticas
                          {!period.directorName && <span className="text-red-500 normal-case font-medium">· requerido para generar documentos</span>}
                        </label>
                        <input
                          type="text"
                        ...
                        {/* Los oficios oficiales imprimen estos tres datos bajo
                            la firma del Responsable. Sin ellos el documento se
                            genera igual, pero con el hueco en blanco. */}
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-1">
                          {([
                            ['directorDni', 'Cédula', 'Ej: 1311920613'],
                            ['directorPhone', 'Teléfono', 'Ej: 0999279120'],
                            ['directorEmail', 'Correo', 'Ej: practicas@uleam.edu.ec'],
                            ...
                            <div key={campo} className="flex flex-col gap-1">
                              ...
                              <input
                                type="text"
                                defaultValue={(period as any)[campo] || ''}
                                disabled={!isAdmin}
                                onBlur={(e) => {
                                  if (e.target.value !== ((period as any)[campo] || '')) {
                                    handleUpdateAuthority(period.id, campo, e.target.value)
                                  }
                                }}
                                placeholder={ejemplo}
                                title={!isAdmin ? 'Solo el Administrador define las autoridades' : 'Se imprime bajo la firma en los oficios'}
                                className="w-full bg-slate-50 border-none rounded-[8px] px-3 py-1.5 text-[13px] focus:ring-2 focus:ring-indigo-100 transition-shadow text-[#0f172a] disabled:opacity-60 disabled:cursor-not-allowed"
                              />
                            </div>
                          ))}
                        </div>
