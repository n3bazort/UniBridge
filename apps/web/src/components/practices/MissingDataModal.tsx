      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-rose-100 w-full max-w-md overflow-hidden flex flex-col">
        ...
        <div className="p-6 flex flex-col gap-4">
          ...
          <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-100 mt-1">
            <Button variant="ghost" onClick={onClose} className="text-xs rounded-xl">
              Cerrar
            </Button>
            {/* destructive = mismo rojo que ya usa todo el modal (encabezado,
                borde, alerta): esto sí es semánticamente "danger" — bloquea
                la emisión de un documento por un impedimento legal/técnico. */}
            <Button
              variant="destructive"
              onClick={() => {
                onClose()
